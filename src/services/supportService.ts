...
export const supportService: SupportService = mode === 'mock' ? mockSupportService : {
  async ask() { throw new Error('This deployment supports mock mode only. Set VITE_SUPPORT_MODE=mock.') },
  createHandoff() { throw new Error('Live handoff is not configured. Use mock mode for this demo.') },
}
